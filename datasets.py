...
class trainset_loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_data = torch.FloatTensor(input_data).unsqueeze_(0) / 3.84
        label_data = torch.FloatTensor(label_data).unsqueeze_(0) / 3.84
        prj_data = torch.FloatTensor(prj_data).unsqueeze_(0) / 3.84
...
class valiset_loader(Dataset):
    def __init__(self, root, dose):
        self.file_path = 'input_' + dose
        self.files_A = sorted(glob.glob(os.path.join(root, 'vali', self.file_path, 'data') + '*.mat'))
